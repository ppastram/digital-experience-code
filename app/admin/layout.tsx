...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebar = <AdminSidebar mobileOpen={mobileOpen} onClose={() => setMobileOpen(false)} />
  const content = (
  ...
  return (
    <div className="flex flex-1 min-h-0" dir="ltr">
      {isRtl ? <>{content}{sidebar}</> : <>{sidebar}{content}</>}
    </div>
  )
}
